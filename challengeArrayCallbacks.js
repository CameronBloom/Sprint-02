...
const animals = [
  { type: 'mouse', size: 'small', weight: 1 },
  { type: 'dog', size: 'small', weight: 10 },
  { type: 'lion', size: 'medium', weight: 150 },
  { type: 'elephant', size: 'big', weight: 5000 },
  { type: 'tiger', size: 'medium', weight: 400 },
  { type: 'bat', size: 'small', weight: 8 },
  { type: 'pig', size: 'small', weight: 40 }
];
...
console.log();
const arrTypes = animals.map(animal => {
  return animal.type;
});
const arrObjects = animals.map(animal => {
  return {
    size: animal.size,
...
console.log(`Objects:`, arrObjects);
...
let arrSmall = animals.filter(animal => animal.size === 'small');
let arrLight = animals.filter(animal => animal.weight < 100);
let arrHeavy = animals.filter(animal => animal.weight > 100);

console.log(`Small:`, arrSmall);
console.log(`Light:`, arrLight);
console.log(`Heavy:`, arrHeavy);
...
const animalWeightTotal = animalWeights.reduce(
  (animal, currentValue) => animal + currentValue, 0
);

console.log(animalWeightTotal);

const animalWeightAverage = animalWeights.map(weight => (animalWeightTotal / (animalWeights.length)).toFixed(2));

console.log(animalWeightAverage);